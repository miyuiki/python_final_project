...
if __name__ == '__main__':
    ...
    board = Entry(win, textvariable=board_text)
    board_text.set(" ")
    board.pack()
    ...
    l2.pack()
    page_text = StringVar()
    page = Entry(win, textvariable=page_text)
    page_text.set(" ")
    page.pack()
    ...
    push_text = StringVar()
    push = Entry(win, textvariable=push_text)
    push_text.set(" ")
    push.pack()


    def on_click():
        board_select = board_text.get()
        page_num = page_text.get()
        push_num = push_text.get()
        c = PttWebCrawler(str(board_select), page_num, int(push_num))
        mixer.init()
        for i in joke:
            print(i)
            with tempfile.NamedTemporaryFile(delete=True) as fp:
                tts = gTTS(text=i, lang='zh')
                tts.save('{}.mp3'.format(fp.name))
                mixer.music.load('{}.mp3'.format(fp.name))
                mixer.music.play()
                time.sleep(len(i) * 3 / 10)

    Button(win, text="開始朗讀", command=on_click).pack()
    win.mainloop()
